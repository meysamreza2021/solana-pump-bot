...
# گام ۲: گرفتن markets برای top ۲۵۰ Solana (batch ۲۵۰ تا، با sleep بیشتر برای rate limit)
rows = []
page = 1
per_page = 100  # کم کردم برای جلوگیری از rate limit
while len(rows) < 1000:
    ...
    for coin in markets:
        ...
        rows.append([id_, f"{symbol}/SOL", symbol, price_change_1h, vol_24h, mc, age_h, contract])

    print(f"Page {page}: {len(markets)} توکن پردازش شد. کل: {len(rows)}")
    page += 1
    time.sleep(2)  # sleep بیشتر
